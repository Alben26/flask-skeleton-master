"""empty message

Revision ID: 4eecb54b8aac
Revises: 36b93d81ad60
Create Date: 2015-03-04 12:04:51.449655

"""

# revision identifiers, used by Alembic.
revision = '4eecb54b8aac'
down_revision = '36b93d81ad60'

from alembic import op
import sqlalchemy as sa


def upgrade():
    ### commands auto generated by Alembic - please adjust! ###
    op.create_table('loguser',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('Ales', sa.String(), nullable=False),
    sa.Column('Postulka', sa.String(), nullable=False),
    sa.Column('pohlavi', sa.Boolean(name='zena'), nullable=True),
    sa.Column('datum_insertu', sa.DateTime(), nullable=True),
    sa.PrimaryKeyConstraint('id', name='pk_loguser'),
    sqlite_autoincrement=True
    )
    op.create_index('ix_loguser_Postulka', 'loguser', ['Postulka'], unique=False)
    op.create_table('cards',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('card_number', sa.String(), nullable=False),
    sa.Column('TIME', sa.DateTime(), nullable=False),
    sa.Column('WTIME', sa.DateTime(), nullable=False),
    sa.PrimaryKeyConstraint('id', name='pk_cards'),
    sqlite_autoincrement=True
    )
    ### end Alembic commands ###


def downgrade():
    ### commands auto generated by Alembic - please adjust! ###
    op.drop_table('cards')
    op.drop_index('ix_loguser_Postulka', table_name='loguser')
    op.drop_table('loguser')
    ### end Alembic commands ###
